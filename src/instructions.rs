...
use crate::extract_instructions::FromBytes;

pub fn call_candidates(instr_iter: impl Iterator<Item = u64>, config: &Config) -> Vec<(u64, usize)> {
...
pub fn ret_candidates(instr_iter: impl Iterator<Item = u64>, config: &Config) -> Vec<(u64, usize)> {
    // Destructure CLI params we need
...
pub fn find_potential_edges(instr_iter: impl Iterator<Item = u64>, call_candidate: u64, config: &Config, instr_count: usize) -> Vec<(usize, usize)> {
    ...
    if *is_absolute_addressing {
        ...
        for (i, instr) in instr_iter.enumerate() {
            if instr & call_opcode_mask == call_candidate {
                ...
                if 0 <= address && address < instr_count as i64
...
pub fn filter_valid_edges(binary: &[u8], ret_opcode: u64, config: &Config, potential_call_edges: &Vec<(usize, usize)>, instr_count: usize) -> Vec<(usize, usize)> {
    let Config { ret_func_dist,
                 ret_opcode_mask,
                 .. } = config;
    
    let mut valid_call_edges: Vec<(usize, usize)> = Vec::new();
    let mut valid: FxHashSet<usize> = Default::default();

    for &(from_edge, to_edge) in potential_call_edges {
        // Already seen
        if valid.contains(&to_edge) {
        ...
        for instr in crate::parse_instructions(&binary[adjusted_to_edge - distance..adjusted_to_edge], &config.endiannes, config.instr_len) {
